		...
		requestGenericSearchFields(new IQResultCallback() {
			...
			public void onIQResult(final IQ response) {
				if (IQ.isSuccess(response)) {
					listener.onSuccess(processFieldsResults(session.getCurrentUserURI(), response.getFirstChild(filterQuery)));
				} else {
					// TODO
			...
			public void onIQResult(final IQ response) {
				if (IQ.isSuccess(response)) {
					...
					if (form.x().equals(NoPacket.INSTANCE)) {
						...
						final SearchFields fieldResults = processFieldsResults(session.getCurrentUserURI(), response.getFirstChild(filterQuery));
